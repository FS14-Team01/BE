...
import formatToKst from "../utils/dateTime.js";

// 응답 형식 포맷 함수
function formatRandomPointDrawResponse(randomPointDraw, unselectedAmounts, points) {
  return {
    randomPointDraw: {
...
async function getMyPoint(userId) {
  ...
  const existingDraw = 
    await pointRepository.findRandomPointDraw(userId, drawDate, period);
...
async function createRandomPointDraw(userId) {
  ...
  if (existingDraw) {
    const error = new Error("현재 시간대의 랜덤 포인트 기회를 이미 사용했습니다.");
    error.status = 409;
  ...
  const unselectedPointList = 
    RANDOM_POINT_WEIGHTS.filter((innerArray) => innerArray[0] !== amount);
  ...
  const unselectedAmounts = Math.random() < 0.5 
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const newRandomPointDraw = 
        await pointRepository.createRandomPointDraw(tx, {
          userId, drawDate, period, amount
        });

      const updatedUser = 
        await pointRepository.incrementUserPoint(tx, userId, amount);
      
      await pointRepository.createPointTransaction(tx, { userId, amount, type: "RANDOM_BOX" });
    ...
    if (
      ...
      const alreadyUsedError = new Error("현재 시간대의 랜덤 포인트 기회를 이미 사용했습니다.");
